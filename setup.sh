...
if ! id -u "$SNB_USER" &>/dev/null; then
    ...
    usermod -aG sudo "$SNB_USER" || usermod -aG wheel "$SNB_USER" # Use wheel for RedHat/CentOS
fi
...
# Move the public key and authorized_keys file if they exist
...
if ! grep -q "^PubkeyAuthentication yes" "$SSHD_CONFIG"; then
    ...
    echo "Added 'PubkeyAuthentication yes' to $SSHD
# Check and update PubkeyAuthentication
if ! grep -q "^PubkeyAuthentication yes" "$SSHD_CONFIG"; then
    echo "PubkeyAuthentication yes" | sudo tee -a "$SSHD_CONFIG"
